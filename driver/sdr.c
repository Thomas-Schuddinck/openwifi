...
static irqreturn_t openwifi_rx_interrupt(int irq, void *dev_id)
{
	...
	spin_lock(&priv->lock);
	while (1)
	{																		   // loop all rx buffers that have new rx packets
...
static irqreturn_t openwifi_tx_interrupt(int irq, void *dev_id)
{
	...
	while (1)
	{ // loop all packets that have been sent by FPGA
		...
		if (reg_val != 0xFFFFFFFF)
		{
			...
			printk(KERN_DEBUG "##################################");
			printk(KERN_DEBUG "TX_INTERRUPT LEGACY: 0x%02x%02x%02x", skb->data[0], skb->data[1], skb->data[2]);
			printk(KERN_DEBUG "TX_INTERRUPT Greenfield: 0x%02x %02x %02x %02x %02x %02x", skb->data[GREENFIELD_OFFSET + 0], skb->data[GREENFIELD_OFFSET + 1], skb->data[GREENFIELD_OFFSET + 2], skb->data[GREENFIELD_OFFSET + 3], skb->data[GREENFIELD_OFFSET + 4], skb->data[GREENFIELD_OFFSET + 5]);
			printk(KERN_NOTICE "0x%02x%02x%02x", skb->data[0], skb->data[1], skb->data[2]);
			printk(KERN_NOTICE "0x%02x%02x%02x%02x%02x%02x", skb->data[GREENFIELD_OFFSET + 0], skb->data[GREENFIELD_OFFSET + 1], skb->data[GREENFIELD_OFFSET + 2], skb->data[GREENFIELD_OFFSET + 3], skb->data[GREENFIELD_OFFSET + 4], skb->data[GREENFIELD_OFFSET + 5]);
			printk(KERN_DEBUG "##################################");
						...
						struct sk_buff *skb)
{
	...
	printk(KERN_DEBUG "TX [TIMESTAMP] : 0x%02x %02x %02x %02x %02x %02x %02x %02x", skb->head[timestamp_absolute_offset + 0], skb->head[timestamp_absolute_offset + 1], skb->head[timestamp_absolute_offset + 2], skb->head[timestamp_absolute_offset + 3], skb->head[timestamp_absolute_offset + 4], skb->head[timestamp_absolute_offset + 5], skb->head[timestamp_absolute_offset + 6], skb->head[timestamp_absolute_offset + 7]);
	...
	if (skb->data_len > 0)
	{ // more data are not in linear data area skb->data
	...
	if (priv->slice_idx == 0xFFFFFFFF)
	{ // use Linux default prio setting, if there isn't any slice config
		queue_idx = prio;
	}
	else
	{	// customized prio to queue_idx mapping
	...
	else if (force_use_cts_protect)
	{						   // could override mac80211 setting here.
	...
	if (((dma_fifo_no_room_flag >> queue_idx) & 1) || ((NUM_TX_BD - ((hw_queue_len >> (queue_idx * 8)) & 0xFF)) < RING_ROOM_THRESHOLD) || ring->stop_flag == 1)
	{
		...
		printk(KERN_NOTICE "%s openwifi_tx: WARNING ieee80211_stop_queue prio %d queue %d no room flag %x hw queue len %08x request %d wr %d rd %d\n", sdr_compatible_str,
	...
	{
		printk(KERN_NOTICE "%s openwifi_tx: WARNING status!=DMA_COMPLETE\n", sdr_compatible_str);
		goto openwifi_tx_early_out_after_lock;
	}

	if ((*(u32 *)(&(skb->data[4]))) || ((*(u32 *)(&(skb->data[12]))) & 0xFFFF0000))
	{
		printk(KERN_NOTICE "%s openwifi_tx: WARNING 1 %d %08x %08x %08x %08x\n", sdr_compatible_str, num_byte_pad, *(u32 *)(&(skb->data[12])), *(u32 *)(&(skb->data[8])), *(u32 *)(&(skb->data[4])), *(u32 *)(&(skb->data[0])));
		goto openwifi_tx_early_out_after_lock;
	}
							...
							const struct ieee80211_tx_queue_params *params)
{
	...
	{
	case 0:
		reg_val = ((reg_val & 0xFFFFFF00) | ((cw_min_exp | (cw_max_exp << 4)) << 0));
		break;
	case 1:
		reg_val = ((reg_val & 0xFFFF00FF) | ((cw_min_exp | (cw_max_exp << 4)) << 8));
		break;
	case 2:
		reg_val = ((reg_val & 0xFF00FFFF) | ((cw_min_exp | (cw_max_exp << 4)) << 16));
		break;
	case 3:
		reg_val = ((reg_val & 0x00FFFFFF) | ((cw_min_exp | (cw_max_exp << 4)) << 24));
		break;
	default:
		printk("%s openwifi_conf_tx: WARNING queue %d does not exist", sdr_compatible_str, queue);
		return (0);
	}
...
// //set ad9361 in certain mode
	...
	rand_val %= 250;
	priv->mac_addr[0] = 0x66;
	priv->mac_addr[1] = 0x55;
	priv->mac_addr[2] = 0x44;
	priv->mac_addr[3] = 0x33;
	priv->mac_addr[4] = 0x22;
